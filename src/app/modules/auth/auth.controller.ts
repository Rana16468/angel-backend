...
const tagUser: RequestHandler = catchAsync(async (req, res) => {
  const result = await AuthServices.tagUserIntoDb(req.query);
  sendResponse(res, {
    success: true,
    statusCode: httpStatus.OK,
    message: "Successfully Find All Teg User ",
    data: result,
  });
});
...
const dashboardCount:RequestHandler=catchAsync(async(req , res)=>{
     ...
     const result=await AuthServices. dashboardCountIntoDb();
       sendResponse(res, {
    success: true,
    statusCode: httpStatus.OK,
    message: "Successfully Find All Dashboard Count ",
    data: result,
  });
});
